...
public class LogisticsCenter {
    ...
    /**
     * Load route map from assets json files generated by the arouter-register plugin
     * for library modules. Each library module emits a {@code <module>.json} describing
     * which generated classes implement IRouteRoot / IInterceptorGroup / IProviderGroup.
     *
     * @return true if at least one class was registered from assets
     */
    private static boolean loadRouterMapFromAssets() {
        try {
            String[] files = mContext.getAssets().list("aroute_reg");
            if (files == null) {
                return false;
            }
            ...
            for (String file : files) {
                if (file.endsWith(".json")) {
                    registered |= parseRoutesJson(file);
                }
            }
            return registered;
        } catch (Exception e) {
            logger.error(TAG, "load router map from assets failed.", e);
            return false;
        }
    }

    /**
     * Parse a single {@code *.json} assets file and register its class list.
     */
    private static boolean parseRoutesJson(String fileName) {
        ...
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                String interfaceName = item.optString("interfaceName");
                if (!isRegistrableInterface(interfaceName)) {
                    continue;
                }
                JSONArray classes = item.optJSONArray("classes");
                if (classes == null) {
                    continue;
                }
                for (int j = 0; j < classes.length(); j++) {
                    String className = classes.optString(j);
                    if (!TextUtils.isEmpty(className)) {
                        register(className);
                        registered = true;
                    }
            ...
            logger.error(TAG, "parse routes json failed: " + fileName, e);
            return false;
        } finally {
            if (input != null) {
                try {
    ...
    private static boolean isRegistrableInterface(String interfaceName) {
        return IRouteRoot.class.getName().equals(interfaceName)
                || IInterceptorGroup.class.getName().equals(interfaceName)
                || IProviderGroup.class.getName().equals(interfaceName);
    }
    ...
    public synchronized static void init(Context context, ThreadPoolExecutor tpe) throws HandlerException {
        ...
        try {
            ...
            } else if (loadRouterMapFromAssets()) {
                logger.info(TAG, "Load router map by arouter-register plugin assets.");
            } else {
